...
const Card = ({ id, price, title, imageUrl, addSneakersToCart, additemToFavorites }) => {
  const { wasAddedInCart, wasAddedInFavorites } = React.useContext(AppContext);

  const objPrams = { price, title, imageUrl, id, parentId: id };

  const clickToFavorites = () => {
    additemToFavorites(objPrams);
  };

  const clickToAdd = () => {
    addSneakersToCart(objPrams);
  };
